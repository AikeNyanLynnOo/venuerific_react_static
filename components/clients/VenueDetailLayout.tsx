...
import { VenueDetailInfoTabs } from "../molecules/VenueDetailInfoTabs";
import { VenueEnquiryFormModal } from "../molecules/VenueEnquiryFormModal";
import { useDataSet } from "../hooks/useDataSet";
import { useFetchData } from "../hooks/useFetchData";
import { useVenueDetailSet } from "../hooks/useVenueDetailSet";
...
export const VenueDetailLayout = ({
  ...
  // fetching venue detail info
  const { data, meta, loading, error } = useFetchData({
    fetcher: getVenueDetailInfo,
    args: {
  ...
  // setting client side venue detail info
  useVenueDetailSet(data, meta, [loading]);
  ...
  const resetFields = () => {
    ...
    if (
      enquiry_options &&
      enquiry_options.room_package &&
      enquiry_options.room_package.length > 0
    ) {
      setRoomId(enquiry_options.room_package[0].value || 0);
    }
    setPackageId("");
  ...
  const handleToggleFavourite = async () => {
    ...
    try {
      ...
      setVenueFavourite(data.favorite);
    } catch (error: any) {
      setIsFavLoading(false);
      setFavErr(error);
    }
  ...
  // room change handler with event
  const handleRoomChange = (e: any) => {
    ...
    setCapacityErr("");
    if (e && e.target.value) {
      const room = room_package.find(
        (pkg: any) => `${pkg.value || 0}` === `${e.target.value || 0}`,
      );

      if (room) {
        setRoomId(room.value || 0);
      }
      if (room && room.packages && room.packages.length > 0) {
        setPackages(room.packages);
      }
      if (room && room.min_capacity) {
        setMinCapacity(room.min_capacity);
      }
      if (room && room.max_capacity) {
        setMaxCapacity(room.max_capacity);
      }
      if (room && room.min_duration) {
        setMinDuration(room.min_duration);
      } else {
        setMinDuration(1);
      }
      if (room && room.max_duration) {
        setMaxDuration(room.max_duration);
      } else {
        setMaxDuration(12);
      }
    }
  ...
  useEffect(() => {
    if (
      ...
      enquiry_options.room_package &&
      enquiry_options.room_package.length > 0 &&
      enquiry_options.room_package[0].packages
    ) {
      setPackages(enquiry_options.room_package[0].packages);
    }
  }, [enquiry_options]);

  // setting room if default
  useEffect(() => {
    if (
      enquiry_options &&
      enquiry_options.room_package &&
              ...
              <div className="w-full ">
                  ...
                  >
                    ...
                    <div className="border rounded-xl p-3">
                      ...
                      <div className="flex justify-between items-center">
                        {(attributes && attributes.venue_price && (
                          <p className="text-base font-normal leading-7">
                            Starts from
                            <span className="font-bold leading-7 ml-2">
                              {attributes.venue_price}
                            </span>
                          </p>
                        )) || (
                          <span className="text-base font-semibold leading-7">
                ...
                <div>
                  <h2 className="text-xl font-semibold leading-8">
                    {country_long} Venues You May Like
                  </h2>
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {similar_venues &&
                      similar_venues.length > 0 &&
                      similar_venues
                        .slice(0, 4)
                        .map((venue, index) => (
                ...
                <div className="flex justify-between items-center w-full lg:hidden fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-2xl border p-4">
                  ...
                  <VenueEnquiryFormModal
                    ...
                    country={country}
                    venueName={
                      (venue && venue.attributes && venue.attributes.name) || ""
                    }
                    // form fields
                    currentUser={current_user}
                    customerGetInTouch={customerGetInTouch}
